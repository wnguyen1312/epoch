...
CONTAINS
  ...
  SUBROUTINE read_field_data_r8(filename, block_id, field_data, grid_x)
  ...
  SUBROUTINE read_particle_data(filename, species_name, particle_data)
    ...
    DO iblock = 1, nblocks
      ...
      IF (TRIM(species_id) == species_name) THEN
        ...
        IF (rank == 0) PRINT*, 'Column 1 is ', block_id
      END IF
    END DO
...
PROGRAM read_sdf
  ...
  CALL read_field_data_r8(filename, block_id, field_data, grid_x)
  ...
  PRINT*, 'Average px on rank ', rank, ' is ', &
          SUM(particle_data(:,2)) / SIZE(particle_data(:,2))

  CALL mpi_finish
  DEALLOCATE(field_data, grid_x, particle_data)
